...
if (!file_exists('Concurso de emprendedurismo')) {
  ...
  if (file_exists('Concurso de emprendedurismo')) {
    if (file_exists('Concurso de emprendedurismo/' . $num_control . '-' . $nombre_alumno . '-' . $id_act . '-' . 'ConcursoEmprendedurismo.pdf')) {
      echo '<script language="javascript">alert("El archivo ya existe");window.location.href="../agregar-credito-user.php"</script>';
    } else {
      if (move_uploaded_file($guardado, 'Concurso de emprendedurismo/' . $num_control . '-' . $nombre_alumno . '-' . $id_act . '-' . 'ConcursoEmprendedurismo.pdf')) {
        //insertar datos en la tabla
        $inserta = "INSERT INTO evidencia(numero_control, id_evento, subido, ruta_doc, responsable) VALUES ('$num_control', '$id_act', 1, 'Concurso de emprendedurismo/$num_control-$nombre_alumno-$id_act-ConcursoEmprendedurismo.pdf', '$responsable')";
